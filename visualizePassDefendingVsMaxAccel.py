...
def main():
    ...
    names_arr, eff_arr, acc_arr = grab_best_players(cursor, 50, 100)
    df = pd.DataFrame(list(zip(eff_arr, acc_arr, names_arr)), columns=['CompletionPercentage', 'MaxAccel', 'PlayerName'])
    fig = px.scatter(df, x='CompletionPercentage', y='MaxAccel', hover_data=['PlayerName'])
    fig.show()
    fig.write_html("passDefendingVsMaxAccel.html")
...
def grab_best_players(cursor, min_attempts, num_players):
    ...
    speed_arr = get_max_accel(cursor, sorted_best_players)
...
def get_max_accel(cursor, names_arr):
    acc_arr = []
    for name in names_arr:
        acc_arr = dataStore.get_max_accel_by_nflId(cursor, name[0])
        acc_arr.append(acc_arr[0][0])
    return acc_arr
